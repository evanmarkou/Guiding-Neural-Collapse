...
import torch
from nc.optimisers.pymanopt.gpu.optimizers.optimizer import Optimizer, OptimizerResult
        ...
        *,
        ...
        if Delta_bar is None:
            try:
                ...
                Delta_bar = torch.sqrt(manifold.dim)
        if Delta0 is None:
            Delta0 = Delta_bar / 8
        ...
        while True:
            ...
            if not self.use_rand:
                ...
                while manifold.norm(x, eta) > Delta:
                    eta = eta = torch.sqrt(torch.sqrt(torch.finfo(torch.float32).eps)) * eta
            ...
            rho_reg = max(1, abs(fx)) * torch.finfo(torch.float32).eps * self.rho_regularization
            ...
            if rho < 1.0 / 4 or not model_decreased or torch.isnan(rho):
                trstr = "TR-"
                Delta = Delta / 4
                consecutive_TRplus = 0
    ...
    def _truncated_conjugate_gradient(
        ...
        for j in range(int(maxinner)):
            ...
            if d_Hd <= 0 or e_Pe_new >= Delta**2:
                ...
                tau = (
                    -e_Pd + torch.sqrt(e_Pd * e_Pd + d_Pd * (Delta**2 - e_Pe))
                ) / d_Pd
