...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        if outdim == 0:
            ...
            self.bn2 = norm_layer(planes, affine=self.affine)
        else:
            self.conv2 = conv3x3(planes, outdim)
            self.bn2 = norm_layer(outdim, affine=self.affine)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = norm_layer(width, affine=self.affine)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width, affine=self.affine)
        if outdim == 0:
            self.conv3 = conv1x1(width, planes*self.expansion)
            self.bn3 = norm_layer(planes*self.expansion, affine=self.affine)
        else:
            self.conv3 = conv1x1(width, outdim)
            self.bn3 = norm_layer(outdim, affine=self.affine)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.tau = kwargs['args'].temperature
        self.decl_ETF = kwargs['args'].declarative_ETF
        self.fixdim = kwargs['args'].fixdim
        self.ETF_fc = kwargs['args'].ETF_fc
        self.device = kwargs['args'].device
        self.bias = kwargs['args'].bias
        self.affine = self.bias
        self.inplanes = 64
        self.dilation = 1
        ...
        if self.fixdim:
            self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                            dilate=replace_stride_with_dilation[2], outdim=num_classes+1)
            num_features = num_classes + 1
        else:
            self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
        ...
        if self.decl_ETF:
            ...
            if not kwargs['args'].inference:
                self.FeaturesMovingAverage = FeaturesMovingAverageLayer(num_features, num_classes, device=self.device)
                self.ClosestETFGeometry = ClosestETFGeometryLayer(num_features, num_classes, device=self.device)
        elif self.ETF_fc:
            self.classifier = nn.Linear(num_features, num_classes, bias=True)
            self.classifier.bias.requires_grad_(False)
        else:
            self.classifier = nn.Linear(num_features, num_classes, bias=self.bias)
                ...
                if self.ETF_fc:
                    ...
                    if P is not None:
                        ...
                        m.weight = nn.Parameter(
                            torch.mm(weight, torch.eye(num_classes, num_features)))
                    m.weight.requires_grad_(False)

        # Zero-initialise the last BN in each residual branch,
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                norm_layer(planes * block.expansion, affine=self.affine),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, affine=self.affine))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks-1):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, affine=self.affine))

        downsample = None
        if outdim != 0:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, outdim),
                norm_layer(outdim, affine=self.affine),
            )

        layers.append(block(self.inplanes, planes, groups=self.groups,
                            base_width=self.base_width, dilation=self.dilation,
                            norm_layer=norm_layer, downsample=downsample, outdim=outdim, affine=self.affine))
    ...
    @torch.no_grad()
    def normalise_classifier(self):
      self.classifier.weight.data.copy_(F.normalize(self.classifier.weight.data))
